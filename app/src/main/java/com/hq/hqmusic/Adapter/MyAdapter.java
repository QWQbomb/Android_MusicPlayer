...
public class MyAdapter extends BaseAdapter {
	...
	// 定义四种颜色，蓝色，紫色，绿色，红色
	private int myBlue = Color.argb(0xff, 0x00, 0xA2, 0xE8);
	private int myPurple = Color.argb(0xff, 0xAF, 0xA3, 0xDC);
	private int myGreen = Color.argb(0xff, 0x87, 0xEB, 0x96);
	private int myRed = Color.argb(0xff, 0xFF, 0x80, 0x80);
	...
	public View getView(int i, View view, ViewGroup viewGroup) {
		...
		if (string_song.length() >= 5
                ...
                string_song.length()).equals(".amr")||string_song.substring(string_song.length() - 4,
                string_song.length()).equals(".aac")||string_song.substring(string_song.length() - 4,
                string_song.length()).equals(".oog")) {
		...
		if (i == position_flag) {
			...
			case "purple":
				...
				break;
			case "green":
				holder.song.setTextColor(myGreen);
				holder.singer.setTextColor(myGreen);
				holder.duration.setTextColor(myGreen);
				holder.position.setText("");
				holder.position
